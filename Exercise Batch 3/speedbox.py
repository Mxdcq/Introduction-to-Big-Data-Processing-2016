...
average_speed = {}
for key in bus_speed:
    total = 0
    for i in range(len(bus_speed[key])):
        total += float(bus_speed[key][i])
    average_speed[key] = total / len(bus_speed[key])

print(average_speed)

with open("average_speed.txt", "a") as f:
    f.write("number" + " " + "speed" + "\n")
    for key in average_speed:
        f.write(key + " " + str(average_speed[key]) + "\n")